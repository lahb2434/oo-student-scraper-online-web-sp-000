...
class Scraper
  ...
  def self.scrape_index_page(index_url)
    ...
    Nokogiri::HTML(open(index_url)).css("div.student-card").each do |person|
      student_index_array << {:name => person.css("div.card-text-container h4.student-name").text, :location => person.css("div.card-text-container p.student-location").text, :profile_url => person.css("a")[0]['href']} 
    end
